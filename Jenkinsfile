      ...
      parallel {
        ...
        stage('Quality Tests') {
          steps {
            ...
            sh 'docker tag nodeapp-dev:trunk timskau/nodeapp-dev:latest'
            sh 'docker push timskau/nodeapp-dev:latest'
          }
        }
      }
      post {
            ...
            steps {
                    retry(3) {
                        timeout(time:10, unit: 'MINUTES') {
                            sh 'docker tag nodeapp-dev:trunk timskau/nodeapp-prod:latest'
                            sh 'docker push timskau/nodeapp-prod:latest'
                            sh 'docker save timskau/nodeapp-prod:latest | gzip > nodeapp-prod-golden.tar.gz'
                        }
                    }
